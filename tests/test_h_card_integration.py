...
def test_profile_accessible_via_user(user, profile):
    """Test that profile is accessible via user relationship."""
    related_profile = user.indieweb_profile

    assert related_profile == profile
    assert related_profile.name == "Test User"
    assert related_profile.h_card["name"] == ["Test User"]


def test_h_card_template_integration(user, profile):
...
def test_h_card_with_micropub_integration(client, user, profile):
    """Test that h-card data can be used in micropub responses."""
    # Create a token for micropub access
    token = Token.objects.create(
        owner=user, client_id="https://example.com/", me="https://example.com/", scope="create"
    )

    # Make a micropub query request
    response = client.get(reverse("indieweb:micropub"), {"q": "config"}, HTTP_AUTHORIZATION=f"Bearer {token.key}")
...
def test_h_card_parsing_integration():
...
def test_profile_admin_integration(client, profile):
...
def test_h_card_without_profile():
...
def test_multiple_profiles_integration(user, profile):
    ...
    assert user.indieweb_profile != user2.indieweb_profile
    assert user.indieweb_profile.name == "Test User"
