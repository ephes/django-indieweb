...
- [ ] Add IndieAuth server metadata and discovery endpoints.
  - Outcome: expose IndieAuth/OAuth authorization-server metadata for django-indieweb deployments, including issuer, authorization endpoint, token endpoint, token introspection endpoint if implemented, supported response types, supported scopes, PKCE challenge methods, and service documentation. Provide a reusable view and URL pattern guidance so host projects can publish ``/.well-known/oauth-authorization-server`` or an equivalent mounted metadata URL without django-indieweb assuming it owns the site root.
  - References: local ``src/indieweb/views.py``, ``src/indieweb/urls.py``, ``docs/indieauth.rst``, ``docs/api.rst``, ``docs/configuration.rst``, ``tests/test_auth_endpoint.py``, and ``tests/test_token_endpoint.py``; Indiekit ``/Users/jochen/src/getindiekit-indiekit/packages/endpoint-auth/lib/controllers/metadata.js``, ``/Users/jochen/src/getindiekit-indiekit/packages/endpoint-auth/index.js``, and ``/Users/jochen/src/getindiekit-indiekit/docs/specifications.md`` at ``1ee20d06``; IndieAuth metadata spec ``https://indieauth.spec.indieweb.org/#indieauth-server-metadata``.
  - Scope exclusions: do not add Indiekit's password setup flow, Pushed Authorization Request storage, or root URL ownership in this slice.
- [ ] Tighten IndieAuth authorization and token wire compatibility.
  - Outcome: audit and align request/response compatibility with current IndieAuth clients where it does not break existing django-indieweb deployments. Cover ``response_type=code`` validation on authorization requests, ``grant_type=authorization_code`` handling on token requests, JSON token/profile responses when requested with ``Accept: application/json``, and ``token_type=Bearer`` in access-token responses. Add ``iss`` in authorization redirects only after the server metadata/issuer work is available. Preserve current legacy-client behavior deliberately where stricter validation would be breaking, and document any compatibility mode.
  - References: local ``src/indieweb/views.py``, ``templates/indieweb/consent.html``, ``tests/test_auth_endpoint.py``, ``tests/test_consent_screen.py``, ``tests/test_token_endpoint.py``, ``docs/indieauth.rst``, and ``docs/api.rst``; Indiekit ``/Users/jochen/src/getindiekit-indiekit/packages/endpoint-auth/lib/controllers/authorization.js``, ``lib/controllers/consent.js``, ``lib/controllers/token.js``, ``lib/middleware/code.js``, and ``test/integration/200-token-grant-json.js`` at ``1ee20d06``; IndieAuth authorization and token response specs ``https://indieauth.spec.indieweb.org/#authorization-request`` and ``https://indieauth.spec.indieweb.org/#access-token-response``.
  - Scope exclusions: do not add refresh tokens, password login, or Indiekit's signed-JWT authorization-code model unless a separate design item justifies it.
...
- [ ] Add Micropub category, channel, and query-discovery support.
  - Outcome: support ``GET /indieweb/micropub/?q=category`` and ``?q=channel`` using host-provided handler configuration, and advertise supported query names through ``q=config`` in the shape common clients expect. Include filtering, ``limit``, and ``offset`` for list-valued configuration where feasible, plus tests for empty/missing handler data and custom ``MicropubContentHandler.get_config()`` overrides.
  - References: local ``src/indieweb/views.py``, ``src/indieweb/handlers.py``, ``tests/test_micropub_endpoint.py``, ``tests/test_micropub_create.py``, ``docs/micropub.rst``, and ``docs/api.rst``; Indiekit ``/Users/jochen/src/getindiekit-indiekit/packages/endpoint-micropub/lib/config.js``, ``lib/controllers/query.js``, ``docs/configuration/publication.md``, and ``docs/specifications.md`` at ``1ee20d06``; Micropub extension issues ``https://github.com/indieweb/micropub-extensions/issues/5``, ``https://github.com/indieweb/micropub-extensions/issues/7``, and ``https://github.com/indieweb/micropub-extensions/issues/40``.
  - Scope exclusions: do not add Indiekit-style publication channel routing or file-path selection in this query slice; handlers remain responsible for interpreting selected channels on create/update.
- [ ] Add Micropub post-list source query pagination and filtering.
  - Outcome: extend the handler interface with an optional list/read method for ``GET ?q=source`` without ``url`` so clients can retrieve a paged list of editable posts. Return ``items`` and ``paging`` data compatible with common Micropub extension clients, support ``after``/``before`` or ``limit``/``offset`` after choosing one documented policy, and keep existing ``?q=source&url=...`` behavior unchanged.
  - References: local ``src/indieweb/views.py``, ``src/indieweb/handlers.py``, ``tests/test_micropub_source.py``, ``tests/test_micropub_endpoint.py``, ``docs/micropub.rst``, and ``docs/api.rst``; Indiekit ``/Users/jochen/src/getindiekit-indiekit/packages/endpoint-micropub/lib/controllers/query.js``, ``lib/post-data.js``, and ``packages/endpoint-micropub/README.md`` at ``1ee20d06``; Micropub extension issues ``https://github.com/indieweb/micropub-extensions/issues/4``, ``https://github.com/indieweb/micropub-extensions/issues/34``, ``https://github.com/indieweb/micropub-extensions/issues/35``, ``https://github.com/indieweb/micropub-extensions/issues/36``, and ``https://github.com/indieweb/micropub-extensions/issues/48``.
  - Scope exclusions: do not add a bundled post database, search index, or host content model; list support should be optional for handlers that can enumerate host-owned content.
