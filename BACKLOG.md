...
- [ ] Sanitize remote Webmention HTML and Webmention author URL fields before display.
  References: `src/indieweb/processors.py`, `src/indieweb/models.py` (`Webmention.author_url`, `Webmention.author_photo`), `src/indieweb/templates/indieweb/webmention_types/{like,reply,mention,repost,nested_response}.html`, `src/indieweb/templates/indieweb/h-card.html`, `src/indieweb/templatetags/webmention_tags.py`, `tests/test_webmention_processor.py`, `tests/test_webmention_templatetags.py`.
  Two related XSS surfaces. (1) Remote `content_html` is stored and rendered with `|safe` in `reply.html:27`, `mention.html:27`, and `nested_response.html:41`. Add a strict allowlist sanitizer that strips dangerous tags/attributes/protocols (scripts, event handlers, forms, SVG, iframes, styles, `javascript:`/`data:` URLs); prefer safe-by-default rendering. (2) `Webmention.author_url` and `Webmention.author_photo` are persisted via `.save()` instead of `full_clean()`, so no `URLValidator` runs and `javascript:`/`data:` schemes reach `<a href>` and `<img src>`. Validate scheme allowlist (http/https) on assignment — either call `full_clean()` from the Webmention processor or attach `URLValidator(schemes=["http","https"])` to those fields. (Django's `URLValidator` rejects `mailto:` regardless of `schemes`; if `mailto:` author URLs need to be supported, write a custom validator that combines `URLValidator` with a `mailto:` short-circuit.) Replace the `mark_safe(f"...")` in `webmention_tags.webmention_endpoint_link` with `format_html`. Add `rel="nofollow noopener ugc"` and `referrerpolicy="no-referrer"` to outbound Webmention author/source links and to the h-card org URL link in `templates/indieweb/h-card.html`. Regression tests must submit `<script>` payloads in `content_html` and `javascript:`/`data:` schemes in author_url/photo. (Note: `Profile.save()` already calls `full_clean()`, so h-card profile URL fields are not affected by surface (2).)

- [ ] Add shared SSRF-safe HTTP handling for Webmention receive/send and WebSub publish/subscribe.
  References: `src/indieweb/http_client.py`, `src/indieweb/processors.py` (`_fetch_source`, `_fetch_vouch`), `src/indieweb/senders.py` (`extract_urls`, `_extract_external_target_urls`, `discover_endpoint`, `fetch_content`, `send_webmention`, `resend_salmentions`), `src/indieweb/websub.py` (`_post_subscription_request`, `notify_hubs`), `src/indieweb/views.py` (`WebmentionEndpoint` source/target validators ~line 1943), `src/indieweb/management/commands/{send_webmentions,notify_websub}.py`, `tests/test_webmention_processor.py`, `tests/test_webmention_sender.py`, `tests/test_websub.py`, `tests/test_send_webmentions_command.py`.
  Every outbound HTTP path uses `httpx` without private-network filtering, and the redirect helper only screens scheme/netloc syntactically. Introduce one shared safe HTTP helper that resolves DNS, rejects loopback / RFC1918 / link-local / multicast / reserved / cloud-metadata IPs (incl. IPv6 mapped variants), connects by IP with `Host` header, and re-applies the check on every redirect. Pin `verify=True`. Restrict the `WebmentionEndpoint` source/target validators to `URLValidator(schemes=["http","https"])` (currently they accept `ftp`/`ftps`). Sub-bullets to land together (or as adjacent PRs sharing the helper):
    - Receive side: `_fetch_source`/`_fetch_vouch` route through the helper; tests cover direct private IPs, localhost names, DNS/redirect-to-private, IPv6 bypass, allowed public URLs.
    - Sender side: apply the SSRF screen to the *target* URL itself before discovery (not just to redirect hops). `_extract_external_target_urls` already filters to absolute external HTTP(S); extend it to reject private/loopback/metadata addresses. Filter non-http schemes inside `extract_urls`. Validate `vouch_url` inside `send_webmention` for callers that bypass the management command. `resend_salmentions` re-pings every historical target forever — add a cooldown, success cutoff, and drop-after-N-failures rule.
    - WebSub side: `_post_subscription_request` and `notify_hubs` route through the helper; document that operator-configured `INDIEWEB_WEBSUB_HUBS` must never be templated from request data.

- [ ] Harden synchronous Webmention and WebSub processing against DoS, decompression bombs, and recursion DoS.
  References: `src/indieweb/views.py`, `src/indieweb/processors.py` (`_fetch_source`, `_fetch_vouch`, `_nested_h_entries`, `_search_for_mentioning_entry`), `src/indieweb/rate_limit.py`, `docs/`, `tests/test_webmention_endpoint.py`, `tests/test_rate_limiting.py`, `tests/test_websub.py`.
  When `INDIEWEB_WEBMENTION_ENQUEUE` is unset, the receive endpoint fetches/parses synchronously with no `max_bytes`. Stream responses with `httpx.Client.stream()`, abort on a configurable byte cap, tighten per-phase timeouts (`httpx.Timeout(connect=3, read=5, write=5, pool=2)`), cap nested h-entry recursion depth (e.g. 8) and candidate count (e.g. 100), document production queue/rate-limit recommendations. The WebSub callback path has the same shape: `request.body` is read fully before the `INDIEWEB_WEBSUB_DELIVERY_MAX_BYTES` check (`views.py:1870`), and the host delivery hook runs inline. Check `Content-Length` first, recommend queued delivery (mirror `INDIEWEB_WEBMENTION_ENQUEUE`), and document a tight `DATA_UPLOAD_MAX_MEMORY_SIZE`.
...
- [ ] Make IndieAuth authorization codes single-use on every failure path.
  References: `src/indieweb/views.py` (`TokenView.post` deletion at line 921; PKCE deletion at line 844; mismatch returns at lines 893/903; logging at line 873), `tests/test_token_endpoint.py`.
  Only PKCE failure and the happy path delete the auth code today; mismatched `redirect_uri` and `scope` paths leave it for replay through the 60s TTL. Call `auth.delete()` on every grant-validation failure inside the token-exchange `try:` block. Redact authorization codes in log records (e.g. `code[:6] + "..."`); tokens are already truncated.
