...
* Documented the current Salmention support status. Ordinary duplicate Webmention reprocessing remains supported, but Salmention sending and receiving are deliberately deferred until the project has source snapshot persistence, nested response storage/display semantics, outbound target tracking, and an explicit resend workflow. No endpoint behavior or settings changed.
* Documented the receive-side Salmention persistence design: future receiving should keep submitted ``Webmention`` rows as top-level source/target notifications, add related source snapshots for fetched HTML and normalized parsed state, add related child rows for stable nested response identities, gate child displayability on the parent remaining verified, suppress duplicate inline children when a direct top-level Webmention represents the same response, and keep all fetch/parse/compare work in processor or worker paths rather than the queued receive endpoint.
